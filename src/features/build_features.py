...
class ProcessData:
    ...
    def _process_gameweek(self, gameweek_df) -> pd.DataFrame:
        ...
        gameweek_df["official_availablity"] = gameweek_df["official_availablity"].map(
            status_dict
        )
    ...
    def _process_predicted_points(self, predicted_points_df) -> pd.DataFrame:
        ...
        predicted_points_df = predicted_points_df.loc[
            :, ~predicted_points_df.columns.str.contains("with_prob")
        ]

        # Rename columns
        # If column name number and contains 'pts', rename to 'GW1_Pts' 'GW2_Pts' etc.
        predicted_points_df.rename(
            columns=lambda x: (
                f"gw_{x.split('_')[0]}_xp"
                if x.split("_")[0].isdigit() and "pts" in x
                else x
            ),
            inplace=True,
        )
        predicted_points_df.rename(
            columns=lambda x: (
                f"gw_{x.split('_')[0]}_prob_of_appearing"
                if x.split("_")[0].isdigit() and x.split("_")[1].__eq__("prob")
                else x
            ),
            inplace=True,
        )
    ...
    def _merge_gameweek_and_predicted_points(
        self, gameweek_df, predicted_points_df
    ) -> pd.DataFrame:
        """
        Summary:
        --------
        ...
        if merged_df.shape[0] != gameweek_df.shape[0]:
            ...
            gameweek_df_ids_not_in_merged_df_ids = set(gameweek_df_ids) - set(
                merged_df_ids
            )
